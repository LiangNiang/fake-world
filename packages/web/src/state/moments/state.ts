...
const injectDefaultFeed = (id: IFeed["id"]): IFeed =>
	INIT_FEEDS.find((v) => v.id === id) ?? { id, ...DEFAULT_FEED };
...
export const feedState = selectorFamily<IFeed, IFeed["id"]>({
	...
	set:
		(id) =>
		({ set, reset, get }, feed) => {
			const { userId: oldUserId, sendTimestamp: oldSendTimestamp } =
				get(allFeedsState).find((v) => v.id === id) ?? {};
			if (feed instanceof DefaultValue) {
				...
				if (oldUserId !== DEFAULT_FEED.userId) {
					set(allFeedsState, (prev) =>
						prev.map((v) =>
							v.id === id
								? { id, userId: DEFAULT_FEED.userId, sendTimestamp: DEFAULT_FEED.sendTimestamp }
								: v,
						),
					);
				}
				...
				if (oldUserId !== userId || oldSendTimestamp !== sendTimestamp) {
					set(allFeedsState, (prev) =>
						prev.map((v) => (v.id === id ? { id, userId, sendTimestamp: sendTimestamp } : v)),
					);
				}
			}
		},
