...
import { INIT_FRIENDS, INIT_MY_PROFILE, MYSELF_ID, generaterFakeUser } from "@/faker/wechat/user";

import { persistAtom } from "../effects";
import type { IFriendsTotalCountDisplay, IProfile } from "./typing";
import { generateNameAnchorGroup } from "./utils";

export const PRIVACY_TEXT_MAP: Record<
	IProfile["privacy"],
	string | ((gender: NonNullable<IProfile["gender"]>, t: TFunction) => string)
> = {
	all: "",
...
const injectDefaultFriendsProfile = (id: IProfile["id"]): IProfile =>
	INIT_FRIENDS.find((v) => v.id === id) ?? generaterFakeUser({ id });
