...
const LoadShareDataSource = () => {
	...
	return (
		<div>
			<div className="font-semibold text-lg">{t("menu.dataSourceManage.loadShare.title")}</div>
			<div className="mt-4">
				<Form layout="inline" form={form} onFinish={loadShare}>
						...
						rules={[
							{ required: true, message: t("menu.dataSourceManage.loadShare.errorEmpty") },
							{
								...
								validator: (_, value) => {
									if (
										value &&
										getRecoil(dataSourceListState).some((item) => item.shareKey === value)
									) {
					...
					>
						<Input
							placeholder={t("menu.dataSourceManage.loadShare.placeholder")}
							className="w-72"
						/>
					</Form.Item>
