...
const NewUserMetaDataEditor = () => {
  ...
  return (
    ...
    >
      <Form.Item<IProfile> name="avatarInfo" label="头像" required rules={[{ required: true, message: '请上传头像' }]}>
        <LocalImageUploadWithPreview />
      </Form.Item>
      <Form.Item<IProfile> name="nickname" label="昵称" required rules={[{ required: true }]}>
        <Input />
      </Form.Item>
      <Form.Item<IProfile> name="remark" label="备注" normalize={(v) => (isEmpty(v) ? undefined : v)}>
        <Input />
      </Form.Item>
      ...
      <Form.Item<IProfile> name="momentsBackgroundLike" label="朋友圈背景图是否点赞" valuePropName="checked">
        <Switch />
      </Form.Item>
          ...
          <>
            {fields.map((field, index) => (
              <Form.Item label={index === 0 ? '电话号码' : ''} key={field.key}>
                ...
                <Button icon={<MinusCircleOutlined />} onClick={() => remove(field.name)} className="ml-2" />
              </Form.Item>
            ))}
            <Form.Item>
