import { Button, DatePicker, Form, Radio, RadioChangeEvent } from 'antd';
...
import { allFeedsState, feedState, IFeed } from '@/state/moments';
...
const AllFeedsMetaDataEditor = ({ index }: EditorProps<unknown, IProfile['id']>) => {
  ...
  const onFinish = (values: IFeed) => {
    ...
    setRecoil(allFeedsState, (prev) => [{ id, userId: values.userId, sendTimestamp: values.sendTimestamp }, ...prev]);
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <Form.Item<IFeed> noStyle shouldUpdate={(pv, cv) => pv.content.type !== cv.content.type}>
        {({ getFieldValue }) => {
          ...
          switch (type) {
            case 'textWithImages':
              return (
                <Form.Item<IFeed> name={['content', 'imagesInfo']} label="图片" tooltip="最多九张图片">
                  <LocalImageUploadWithPreview maxImagesCount={9} />
                </Form.Item>
              );
            case 'video':
              return (
                <Form.Item<IFeed> name={['content', 'videoInfo']} label="视频" tooltip="视频和图片一样，仅仅只是展示的时候加一个播放图标">
      ...
      <Form.Item<IFeed> noStyle shouldUpdate={(pv, cv) => pv.likeUserIds?.length !== cv.likeUserIds?.length}>
        {({ getFieldValue }) =>
