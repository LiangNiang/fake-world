...
import { activatedNodeState, MetaDataType, nodeFreshDataState, nodeInjectMetaState } from '@/state/detectedNode';
...
import { ConversationItemMetaDataEditor, ConversationListMetaDataEditor } from './ConversationMetaDataEditor';
...
export const MetaDataEditor = () => {
  ...
  const renderMetaDataEditorBySingleMetaData = useCallback(
    (metaData?: StaticMetaData.InjectMetaData, freshDataIndex?: number) => {
      ...
      const UsedComponent: (props: EditorProps) => JSX.Element = TYPE_MAP_COMPONENT[type as keyof typeof TYPE_MAP_COMPONENT];
      if (!UsedComponent) {
        return <>该节点没有配置编辑表单</>;
      }
      return (
        <UsedComponent
          data={Array.isArray(nodeData) && freshDataIndex !== undefined ? nodeData[freshDataIndex] : nodeData}
