...
const LocalImageUploadWithPreview = ({ value, onChange, maxImagesCount, ...rest }: Props) => {
  ...
  const handleInputChange = async (ev: ChangeEvent<HTMLInputElement>) => {
    if (isMultiple) {
      ...
      setPreviewFileSrc((prev) => [...(prev as string), ...files.map((v) => URL.createObjectURL(v))]);
  ...
  return (
    <div className="flex flex-col">
      {contextHolder}
      <Input {...rest} type="file" multiple={isMultiple} onChange={handleInputChange} disabled={inputDisabled} />
      <div className="mt-2">
        {isMultiple ? (
          <ReactSortable
          ...
          >
            {(previewFileSrc as string[]).map((v, i) => (
              <div key={i} className="relative mb-1 mr-1">
                <Button
                  className="absolute right-0 top-0 z-10"
                  icon={<DeleteTwoTone />}
                  onClick={() => {
                    onChange?.(value.filter((_, index) => index !== i));
                    setPreviewFileSrc((prev) => (prev as string[]).filter((_, index) => index !== i));
                  }}
                />
          ...
          <HashAssets src={previewFileSrc as string} className="h-28 w-28 overflow-hidden" useAntdImageComponent />
        )}
      </div>
    </div>
  );
