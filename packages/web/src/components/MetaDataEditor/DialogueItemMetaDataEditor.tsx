...
import { dialogueItemState, IDialogueItem } from '@/state/dialogueState';
...
const DialogueItemMetaDataEditor = ({ data }: EditorProps<IDialogueItem, IDialogueItem['id']>) => {
  ...
  return (
    ...
    >
      ...
      <Form.Item<FormValuesType> name="lastMessage" label="最后一条消息" rules={[{ required: true }]}>
        <Input />
      </Form.Item>
      <Form.Item<FormValuesType> name="lastMessageTime" label="最后一条消息发送时间" rules={[{ required: true }]}>
        <Input
          suffix={
            <>
              <Button
