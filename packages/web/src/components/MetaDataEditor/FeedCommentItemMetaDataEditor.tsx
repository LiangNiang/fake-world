...
const FeedCommentItemMetaDataEditor = ({ data, index }: EditorProps<IFeedComment, [IFeed['id'], IFeedComment['id']]>) => {
  ...
  const onFinish = (values: IFeedComment) => {
    setFeed((prev) => ({
      ...prev,
      comments: prev.comments?.map((comment) => (comment.id === commentId ? { ...comment, ...values } : comment)),
    }));
  };
