...
import { IFeedBase, userFeedsState } from '@/state/moments';
...
const PersonalMoments = () => {
  ...
  const allYearKeys = keys(groupedFeeds).filter((v) => v !== TimeType.TODAY && v !== TimeType.YESTERDAY);

  const getYearGroupedFeeds = (year: string) => groupBy(groupedFeeds[year] ?? [], (v) => dayjs(v.sendTimestamp).format('MM-DD'));

  const renderGroupedDateFeeds = (data: Record<string, (IFeedBase & { timeType: ReturnType<typeof getTimeType> })[]>) => {
  ...
  const renderBottomTipsContent = () => {
    switch (momentsPrivacy) {
      case 'all':
        return <div className="mx-2 h-1 w-1 rounded-full bg-black/10"></div>;
      case 'lastSixMonths':
  ...
  const notCurrentYearKeys = allYearKeys.filter((v) => !dayjs(v).isCurrentYear()).sort((a, b) => Number(b) - Number(a));
  ...
  return (
    ...
    >
      {!isEmpty(signature) && <div className="break-words px-4 text-right text-sm text-black/60">{signature}</div>}
      <div className="mt-6 flex flex-col space-y-6">
        <div className="flex flex-col space-y-5 px-3">
          {!isEmpty(todayFeeds) && (
            <div className="flex space-x-3">
              <div className="w-16 text-2xl font-bold">今天</div>
              <div className="flex flex-1 flex-col space-y-1">
                {todayFeeds.map((v) => (
                  <PersonalContent id={v.id} key={v.id} />
                ))}
          ...
          {!isEmpty(yesterdayFeeds) && (
            <div className="flex space-x-3">
              <div className="w-16 text-2xl font-bold">昨天</div>
              <div className="flex flex-1 flex-col space-y-1">
        ...
        {notCurrentYearKeys.map((y) => {
          ...
          return (
            <div key={y}>
              <div className="px-2 text-3xl font-bold">{y}年</div>
              <div className="mt-6 flex flex-col space-y-5 px-3">{renderGroupedDateFeeds(groupedYearData)}</div>
            </div>
          );
        })}
        <canBeDetected.div
