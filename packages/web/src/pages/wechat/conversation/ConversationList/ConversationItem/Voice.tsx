...
import { EConversationRole, IConversationTypeVoice } from '@/state/conversationState';
...
const Voice = ({ senderId, upperText, duration, role, isRead }: Props) => {
  return (
    <CommonBlock
      senderId={senderId}
      upperText={upperText}
      ...
      extraElement={
        ...
        role === EConversationRole.friend && (
          <div className="absolute -right-5 top-1/2 -translate-y-1/2">
            <div className="h-2 w-2 rounded-full bg-wechatRed-3"></div>
          </div>
        )
      }
    >
      <div className={twJoin('flex h-6', role === EConversationRole.mine ? 'flex-row-reverse' : 'flex-row')}>
        <VoiceSVG width={28} height={28} className={twJoin('self-center', role === EConversationRole.mine && 'rotate-180')} />
