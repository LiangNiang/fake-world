...
import { IConversationTypeTransfer } from '@/state/conversationState';
...
const SVG_COMPONENT_MAP: Record<IConversationTypeTransfer['transferStatus'], ComponentType<SVGProps<SVGSVGElement>>> = {
...
const Transfer = ({ upperText, senderId, transferStatus, amount, note, originalSender, role }: Props) => {
  const SVGComp = SVG_COMPONENT_MAP[transferStatus];
  const getTransferNote = () => {
    if (transferStatus === 'awaiting') {
      return isEmpty(note) ? get(TRANSFER_TEXT_NOTE_MAP, [originalSender, role, 'awaiting'], '') : note;
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col pl-1 text-white">
        <div className="flex flex-1 items-center border-b border-white/10 pb-2">
          ...
          <div className="ml-1 flex h-10 flex-col justify-between overflow-hidden">
            ...
            <span className="line-clamp-1 text-xs font-light">{getTransferNote()}</span>
          </div>
        </div>
        <span className="pt-1 text-xs font-light">微信转账</span>
      </div>
    </CommonBlock>
  );
};
