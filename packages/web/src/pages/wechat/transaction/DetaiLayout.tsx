  ...
  return (
    <>
      <div className="flex justify-between px-4 py-2">
        <CloseOutlinedSVG fill="black" className="cursor-pointer" onClick={() => navigate('/wechat')} />
        <div>全部账单</div>
      </div>
      <div className="flex flex-1 flex-col overflow-auto">
        <canBeDetected.div metaData={metaData ?? undefined}>
          <div className="mx-6 mt-8 flex flex-col items-center border-b border-black/10 pb-10">{topElement}</div>
        ...
        <div className="flex flex-1 flex-col bg-[rgba(237,237,237,1)] text-sm">
          <div className="mt-1.5 bg-white px-6">
            <div className="border-b border-black/10 py-3">账单服务</div>
          ...
          <div className={twJoin('mb-8 mt-auto pt-8 text-center text-xs text-black/30')}>本服务由财付通提供</div>
        </div>
      </div>
    </>
  );
