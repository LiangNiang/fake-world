...
import { InjectProps } from '@/components/NodeDetected';
...
import Columns, { TColumnContainerProps } from './Column';
import DetaiLayout from './DetaiLayout';
import Top, { TTopProps } from './Top';
...
function generateColumnConfig(type: TTransactionType, data: unknown): TColumnContainerProps | null {
  switch (type) {
    ...
    case 'credit-card-repayments': {
      const { timestamp, payentMethod, code, merchantCode } = data as ITransactionCreditCardRepayments;
      const timeStr = dayjs(timestamp).format(COMMON_TIME_FORMAT_STR);
      return {
