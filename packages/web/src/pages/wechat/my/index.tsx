...
const My = () => {
  ...
  return (
    <>
      <canBeDetected.div
        className="flex cursor-pointer flex-col pb-6 pl-9 pr-3 pt-12"
        onClick={() => {
          navigate('/wechat/my/profile-edit');
        }}
        metaData={{ type: MetaDataType.MyProfile, treeItemDisplayName: (data) => `个人信息编辑（${data.nickname}）` }}
      >
        <div className="flex">
          ...
          <div className="ml-4 flex flex-col justify-between py-1">
            ...
            <div className="space-x-3 text-sm text-black/60">
        ...
        <div className="ml-20 mt-[2px] flex items-center">
          <div className="flex origin-left scale-[0.85] items-center space-x-[2px] rounded-2xl border border-black/10 px-3">
            ...
            <span className="text-sm text-black/60">{t('wechatPage.my.status')}</span>
          </div>
      ...
      <div className="flex-1 bg-[rgba(237,237,237,1)]">
        <List className="mt-1.5">
          <List.Item withJump icon={<PAYLOGO_OUTLINED_SVG fill="#39CD80" />} onClick={() => navigate('/wechat/service')}>
