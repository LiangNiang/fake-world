  ...
  return (
    <>
      <div className="grid grid-cols-3 bg-[rgba(237,237,237,1)] px-4 py-2">
        <BackFilledSVG fill="black" className="h-5 w-5 cursor-pointer" onClick={() => navigate('/wechat/service')} />
        <div className="flex items-center justify-center font-medium">{t('wechatPage.wallet.title')}</div>
      ...
      <div className="flex flex-1 flex-col overflow-auto bg-[rgba(237,237,237,1)]">
        <List>
          <List.CanBeDetectedItem
            ...
            metaData={{ type: MetaDataType.Wallet, treeItemDisplayName: (data) => `零钱余额：¥${data.balance}` }}
          >
          ...
          <List.CanBeDetectedItem
            ...
            metaData={{ type: MetaDataType.Wallet, treeItemDisplayName: (data) => `零钱通余额：¥${data.miniFund}` }}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div>{t('wechatPage.wallet.mf')}</div>
                <div className="ml-2 rounded-sm bg-wechatOrange-5 px-1 text-xs text-wechatOrange-3">
